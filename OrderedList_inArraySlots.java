...
public class OrderedList_inArraySlots
	...
	for(int index = 0; index < unordered.size(); index++) {
	    int i = champIndex(unordered,index);
	    int firstEl = unordered.get(index);
	    unordered.set(index, unordered.get(i));
	    unordered.set(i, firstEl);
	}
    ...
    /** 
      ...
      @return the index of the smallest element starting at a starting index
     */
    private int champIndex(ArrayList<Integer> challengers, int startIndex) {
	 int minIndex = startIndex;
	 int min = challengers.get(startIndex);
	 for(int index = startIndex ; index < challengers.size(); index++) {
	     if (challengers.get(index) <= min) {
		 min = challengers.get(index);
		 minIndex = index;
	     }
	 }
	 return minIndex;
